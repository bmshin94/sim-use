# sim-use (lycorp-jp/sim-use)

## 프로젝트 개요
LINE과 야후재팬이 공개한, 웹페이지를 사람처럼 직접 둘러보고 클릭하며 실제 사용성을 시뮬레이션하는 "지능형 웹 사용자 에뮬레이터"
사용자가 겪을 수 있는 UI 동선 오류나 디자인 불편 사항을 가상 에이전트가 미리 탐색하고 리포트
복잡한 웹 서비스의 QA 검증과 사용자 경험 테스트를 완전 자동으로 수행하여 서비스 품질을 극대화
